...
/**
 * Base class for DockFX events. Each DockFX event has an associated event source, event target and an event type. The event source specifies for an event handler the object on which that handler has been registered and which sent the event to it. The event target defines the path through which the event will travel when posted. The event type provides additional classification to events of the same {@code DockEvent} class. Like a {@link MouseEvent} the event will be associated with an x and y coordinate local to the node as well as to the screen.
 *
 * @since DockFX 0.1
 */
public class DockEvent extends Event {
    ...
    /**
     * This event occurs when a dock window is being dragged by its title bar and the mouse enters a node's bounds. Unlike a {@code DragEvent} the dock over event is handed to all stages that may be interested in receiving the dock pane.
     */
    public static final EventType<DockEvent> DOCK_ENTER
            = new EventType<DockEvent>(DockEvent.ANY, "DOCK_ENTER");

    /**
     * This event occurs when a dock window is being dragged by its title bar and the mouse is contained in a node's bounds. Unlike a {@code DragEvent} the dock over event is handed to all stages that may be interested in receiving the dock pane.
     */
    public static final EventType<DockEvent> DOCK_OVER
            = new EventType<DockEvent>(DockEvent.ANY, "DOCK_OVER");

    /**
     * This event occurs when a dock window is being dragged by its title bar and the mouse exits a node's bounds. Unlike a {@code DragEvent} the dock over event is handed to all stages that may be interested in receiving the dock pane.
     */
    public static final EventType<DockEvent> DOCK_EXIT
            = new EventType<DockEvent>(DockEvent.ANY, "DOCK_EXIT");

    /**
     * This event occurs when a dock window is being dragged by its title bar and the mouse is released over a node's bounds. Unlike a {@code DragEvent} the dock over event is handed to all stages that may be interested in receiving the dock pane.
     */
    ...
    /**
     * Horizontal x position of the event relative to the origin of the {@code Scene} that contains the DockEvent's node. If the node is not in a {@code Scene}, then the value is relative to the boundsInParent of the root-most parent of the DockEvent's node.
     */
    private final double sceneX;

    /**
     * Returns horizontal position of the event relative to the origin of the {@code Scene} that contains the DockEvent's source. If the node is not in a {@code Scene}, then the value is relative to the boundsInParent of the root-most parent of the DockEvent's node. Note that in 3D scene, this represents the flat coordinates after applying the projection transformations.
     *
     * @return horizontal position of the event relative to the origin of the {@code Scene} that contains the DockEvent's source
     */
    ...
    /**
     * Vertical y position of the event relative to the origin of the {@code Scene} that contains the DockEvent's node. If the node is not in a {@code Scene}, then the value is relative to the boundsInParent of the root-most parent of the DockEvent's node.
     */
    private final double sceneY;

    /**
     * Returns vertical position of the event relative to the origin of the {@code Scene} that contains the DockEvent's source. If the node is not in a {@code Scene}, then the value is relative to the boundsInParent of the root-most parent of the DockEvent's node. Note that in 3D scene, this represents the flat coordinates after applying the projection transformations.
     *
     * @return vertical position of the event relative to the origin of the {@code Scene} that contains the DockEvent's source
     */
     ...
     * @param pickResult pick result. Can be null, in this case a 2D pick result without any further values is constructed based on the scene coordinates
     */
