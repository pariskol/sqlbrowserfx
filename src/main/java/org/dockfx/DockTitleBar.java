...
/**
 * Base class for a dock node title bar that provides the mouse dragging functionality, captioning, docking, and state manipulation.
 *
 * @since DockFX 0.1
 */
...
//		label.graphicProperty().bind(dockNode.graphicProperty());
    ...
    /**
     * The button used for detaching, maximizing, or restoring this title bar and its associated dock node.
     *
     * @return The button used for detaching, maximizing, or restoring this title bar and its associated dock node.
     */
    ...
    /**
     * The mouse location of the original click which we can use to determine the offset during drag. Title bar dragging is asynchronous so it will not be negatively impacted by less frequent or lagging mouse events as in the case of most current JavaFX implementations on Linux.
     */
    ...
    /**
     * The task that is to be executed when the dock event target is picked. This provides context for what specific events and what order the events should be fired.
     *
     * @since DockFX 0.1
     */
    ...
    /**
     * Traverse the scene graph for all open stages and pick an event target for a dock event based on the location. Once the event target is chosen run the event task with the target and the previous target of the last dock event if one is cached. If an event target is not found fire the explicit dock event on the stage root if one is provided.
     *
    ...
    private void pickEventTarget(Point2D location, EventTask eventTask, Event explicit) {
        ...
        for (DockPane dockPane : dockPanes) {
            ...
            while (!stack.isEmpty()) {
                ...
                // if this parent contains the mouse click in screen coordinates in its local
                // bounds
    ...
    public void handle(MouseEvent event) {
        if (event.getEventType() == MouseEvent.MOUSE_PRESSED) {
            if (dockNode.isFloating() && event.getClickCount() == 2 && event.getButton() == MouseButton.PRIMARY) {
                ...
                // drag detected is used in place of mouse pressed so there is some threshold
                // for the
                // dragging which is determined by the default drag detection threshold
                dragStart = new Point2D(event.getX(), event.getY());
            }
        } else if (event.getEventType() == MouseEvent.DRAG_DETECTED && dragAllowed) {
            if (!dockNode.isFloating()) {
                ...
                if (!dockNode.isCustomTitleBar() && dockNode.isDecorated()) {
                    ...
                    // custom do not move if node is docked uncomment for default
                    //dockNode.setFloating(true);
                }
                ...
                // Please note that setMaximized is ruined by width and height changes occurring
                // on the
                // stage and there is currently a bug report filed for this though I did not
                // give them an
                // accurate test case which I should and wish I would have. This was causing
                // issues in the
                // original release requiring maximized behavior to be implemented manually by
                // saving the
                // restored bounds. The problem was that the resize functionality in
                // DockNode.java was
            ...
            dragging = true;
            event.consume();
            // custom do not track event if node is docked , for default remove &&
            // dockNode.isFloating()
        } else if (event.getEventType() == MouseEvent.MOUSE_DRAGGED && dockNode.isFloating() && dragAllowed) {
            if (dockNode.isFloating() && event.getClickCount() == 2 && event.getButton() == MouseButton.PRIMARY) {
            ...
            DockEvent dockEnterEvent = new DockEvent(this, DockEvent.NULL_SOURCE_TARGET, DockEvent.DOCK_ENTER,
