...
/**
 * Base class for a dock pane that provides the layout of the dock nodes. Stacking the dock nodes to the center in a TabPane will be added in a future release. For now the DockPane uses the relative sizes of the dock nodes and lays them out in a tree of SplitPanes.
 *
 * @since DockFX 0.1
 */
public class DockPane extends StackPane implements EventHandler<DockEvent> {
    ...
    private List<SplitPane> splitPanes = new ArrayList<>();
    ...
    /**
     * Whether a DOCK_ENTER event has been received by this dock pane since the last DOCK_EXIT event was received.
     */
    ...
    /**
     * The docking area of the current dock indicator button if any is selected. This is either the root or equal to dock node drag.
     */
    ...
    /**
     * The timeline used to animate the borer of the docking area indicator shape. Because JavaFX has no CSS styling for timelines/animations yet we will make this private and offer an accessor for the user to programmatically modify the animation or disable it.
     */
    ...
    /**
     * The grid pane used to lay out the local dock indicator buttons. This is the grid used to lay out the buttons in the circular indicator.
     */
    private GridPane dockPosIndicator;
    /**
     * The popup used to display the local dock indicator buttons. This allows these indicator buttons to be displayed outside the window of this dock pane.
     */
    private Popup dockIndicatorPopup;

    /**
     * Base class for a dock indicator button that allows it to be displayed during a dock event and continue to receive input.
     *
     * @since DockFX 0.1
     */
    public class DockPosButton extends Button {
         ...
         * Whether this dock indicator button is used for docking a node relative to the root of the dock pane.
         *
         * @param dockRoot Whether this indicator button is used for docking a node relative to the root of the dock pane.
         */
         ...
         * Whether this dock indicator button is used for docking a node relative to the root of the dock pane.
         *
         * @return Whether this indicator button is used for docking a node relative to the root of the dock pane.
         */
    ...
    /**
     * A collection used to manage the indicator buttons and automate hit detection during DOCK_OVER events.
     */
    private ObservableList<DockPosButton> dockPosButtons;

    private DockPosButton dockTop;

    private DockPosButton dockRight;

    private DockPosButton dockBottom;

    private DockPosButton dockLeft;

    private DockPosButton dockTopRoot;

    private DockPosButton dockRightRoot;

    private DockPosButton dockBottomRoot;

    private DockPosButton dockLeftRoot;

    /**
     * Creates a new DockPane adding event handlers for dock events and creating the indicator overlays.
     */
    public DockPane() {
        super();
        ...
        // 12 is the cumulative offset of the stroke dash array in the default.css style
        // sheet
        ...
        dockTopRoot.prefWidthProperty().bind(dockRootPane.widthProperty());
        dockBottomRoot.prefWidthProperty().bind(dockRootPane.widthProperty());
        dockRightRoot.prefHeightProperty().bind(dockRootPane.heightProperty());
        dockLeftRoot.prefHeightProperty().bind(dockRootPane.heightProperty());
    ...
    /**
     * The Timeline used to animate the docking area indicator in the dock indicator overlay for this dock pane.
     *
     * @return The Timeline used to animate the docking area indicator in the dock indicator overlay for this dock pane.
     */
...
//		StyleManager.getInstance().addUserAgentStylesheet(DockPane.class.getResource("default.css").toExternalForm());
    ...
    /**
     * A cache of all dock node event handlers that we have created for tracking the current docking area.
     */
    private ObservableMap<Node, DockNodeEventHandler> dockNodeEventFilters = FXCollections.observableHashMap();

    /**
     * A wrapper to the type parameterized generic EventHandler that allows us to remove it from its listener when the dock node becomes detached. It is specifically used to monitor which dock node in this dock pane's layout we are currently dragging over.
     *
     * @since DockFX 0.1
     */
    private class DockNodeEventHandler implements EventHandler<DockEvent> {
         ...
         *
         * @param node The node that is to listen for docking events and report to the encapsulating docking pane.
         */
    ...
    public List<SplitPane> getSplitPanes() {
        return splitPanes;
    }

    public SplitPane dock(Node node, DockPos dockPos, Node sibling) {
        return dock(node, dockPos, sibling, null);
    }

    /**
     * Dock the node into this dock pane at the given docking position relative to the sibling in the layout. This is used to relatively position the dock nodes to other nodes given their preferred size.
     *
     ...
     */
    public SplitPane dock(Node node, DockPos dockPos, Node sibling, double[] dividers) {
        ...
        if (dockTabPaneParent instanceof SplitPane || dockPos != DockPos.CENTER) {
            ...
            if (split.getItems().size() > 1) {
                ...
                if (split == root && sibling == root) {
                    ...
                    double[] tempDividers = split.getDividerPositions();
                    split.getItems().set(split.getItems().indexOf(sibling), splitPane);
                    split.setDividerPositions(tempDividers);
...
//		}
    ...
    public SplitPane dock(Node node, DockPos dockPos) {
        return dock(node, dockPos, root, null);
    }

    /**
     * Dock the node into this dock pane at the given docking position relative to the root in the layout. This is used to relatively position the dock nodes to other nodes given their preferred size.
     *
     * @param node The node that is to be docked into this dock pane.
     * @param dockPos The docking position of the node relative to the sibling.
     */
    public SplitPane dock(Node node, DockPos dockPos, double[] dividers) {
        return dock(node, dockPos, root, dividers);
    }
