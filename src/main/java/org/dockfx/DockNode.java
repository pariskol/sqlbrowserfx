...
/**
 * Base class for a dock node that provides the layout of the content along with a title bar and a styled border. The dock node can be detached and floated or closed and removed from the layout. Dragging behavior is implemented through the title bar.
 *
 * @since DockFX 0.1
 */
public class DockNode extends VBox implements EventHandler<MouseEvent> {
    ...
    private BooleanProperty maximizedProperty = new SimpleBooleanProperty(false) {
        ...
        protected void invalidated() {
            ...
            // TODO: This is a work around to fill the screen bounds and not overlap the
            // task bar when
            // the window is undecorated as in Visual Studio. A similar work around needs
            // applied for
     ...
     * @param title The caption title of this dock node which maintains bidirectional state with the title bar and stage.
     * @param graphic The caption graphic of this dock node which maintains bidirectional state with the title bar and stage.
     */
    ...
    /**
     * The stage style that will be used when the dock node is floating. This must be set prior to setting the dock node to floating.
     *
    ...
    /**
     * Changes the title bar in the layout of this dock node. This can be used to remove the dock title bar from the dock node by passing null.
     *
     * @param dockTitleBar null The new title bar of this dock node, can be set null indicating no title bar is used.
     */
     ...
     * @param translation null The offset of the node after being set floating. Used for aligning it with its layout bounds inside the dock pane when it becomes detached. Can be null indicating no translation.
     */
    public void setFloating(boolean floating, Point2D translation, Double width, Double height) {
        if (floating && !this.isFloating()) {
            ...
            // offset the new stage to cover exactly the area the dock was local to the
            // scene
    ...
    /**
     * The dock pane that was last associated with this dock node. Either the dock pane that it is currently docked to or the one it was detached from. Can be null if the node was never docked.
     *
    ...
    /**
     * The stage associated with this dock node. Can be null if the dock node was never set to floating.
     *
    ...
    /**
     * The border pane used to parent this dock node when floating. Can be null if the dock node was never set to floating.
     *
     * @return The stage associated with this node.
     */
    ...
    /**
     * Boolean property maintaining whether this node is currently using a custom title bar. This can be used to force the default title bar to show when the dock node is set to floating instead of using native window borders.
     *
     * @defaultValue true
     */
    ...
    /**
     * Boolean property maintaining whether this node is currently docked. This is used by the dock pane to inform the dock node whether it is currently docked.
     *
     * @defaultValue false
     */
     ...
     */
    public SplitPane dock(DockPane dockPane, DockPos dockPos, Node sibling) {
        dockImpl(dockPane);
        return dockPane.dock(this, dockPos, sibling, null);
    }
    ...
    public SplitPane dock(DockPane dockPane, DockPos dockPos) {
        return dock(dockPane, dockPos, new double[]{});
    }
     ...
     * @param dockPos The docking position relative to the sibling of the dock pane.
     */
    public SplitPane dock(DockPane dockPane, DockPos dockPos, double[] weight) {
        dockImpl(dockPane);
    ...
    public void handle(MouseEvent event) {
        ...
        if (event.getEventType() == MouseEvent.MOUSE_PRESSED) {
            ...
            // TODO: find a way to do this synchronously and eliminate the flickering of
            // moving the stage
            // around, also file a bug report for this feature if a work around can not be
            // found this
